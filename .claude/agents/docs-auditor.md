...
--help`, counts against the source derived live (MCP tools via
`grep -c '@mcp.tool' src/bokken/mcp/server.py`, capabilities via
`ls openspec/specs | wc -l`, kata moves via `len(MVP_MOVES)` in kata/moves.py) —
never trust a hardcoded number, the event taxonomy against journal/schema.py
TAXONOMY+EXTENSION_KEYS,
