...
## Purpose
The library is Bokken organizational memory: one summary record per finalized run, scoped by product, so the next run starts from what earlier runs supported, contradicted, or found broken - always with session provenance, never laundered into fresh evidence.
