...
- [ ] 1.1 Add an `OpportunityMatrix` shape to `src/bokken/contract.py`: a
...
- [ ] 2.1 In `src/bokken/report/context.py`, derive the `OpportunityMatrix`
...
- [ ] 2.2 Carry the session's simulated flag onto the matrix. Return an empty /
  omitted matrix when no outcomes were scored.

## 3. CLI verb

- [ ] 3.1 Add a `@guarded` `bokken opportunities <name>` verb to
...
- [ ] 3.2 Exit 2 with a specific message when the session has no scored
  outcomes; unknown session exits 2 per the existing output discipline.

## 4. Report rendering

- [ ] 4.1 In `src/bokken/report/page.py`, render the "Underserved by segment"
...
- [ ] 4.2 In `src/bokken/report/deck.py`, add the matching slide.

## 5. Tests and verification

- [ ] 5.1 Tests: matrix computed with per-cell sample sizes; an `n<2` cell
  ...
  either surface.
- [ ] 5.2 `make check` green (ruff + pytest + `openspec validate --strict`).
