...
- `interview.*`: `interview.consent_requested` — an outbound contact asking a real human to take part, with the participant label and the channel, journaled before the channel reaches them; `interview.consent_resolved` — what came back (`outcome` ∈ `granted | declined | no_response | ambiguous` plus the `basis` in words), with `refs` to the request. Reaching a real human is never off the record, and a consent exchange is never evidence.
