...
- [x] 1.1 `src/bokken/backlog.py`: build the ranked backlog from the dossier
...
- [x] 1.2 flip-the-verdict line: derived from the register versus the test
...
- [x] 1.3 csv and markdown renderers (issue-tracker checklist) in the helper

## 2. Contract

- [x] 2.1 `BacklogResult` (and per-item shape) in `src/bokken/contract.py`,
...
- [x] 3.1 `bokken backlog <name>` in `src/bokken/cli/app.py` (`@guarded`):
  default terminal table, `--json` (BacklogResult), `--format csv|markdown`;
  exit 2 on unknown session

## 4. Tests

- [x] 4.1 offline tests: ranked ordering by impact x uncertainty; supported
